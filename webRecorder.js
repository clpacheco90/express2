var config = require('./cloudConfig'),
	...
	blobService = azure.createBlobService(config.azure.storageName, config.azure.key1);
...
function _upload(response, file) {    
    ...
    //console.log('Creating File at server');
	//fs.writeFileSync(filePath, fileBuffer); localserver test
    console.log('Creating Buffer file');
	var stream = streamifier.createReadStream(fileBuffer);    
    console.log('1. Creating Container');
    blobService.createContainerIfNotExists(config.azure.containerName, function(err, result, resp) {
        if (err) {
            console.log("Couldn't create container %s", config.azure.containerName);
            console.error(err);
        } else {
            if (result) {
                console.log('Container %s created', config.azure.containerName);
            } else {
                console.log('Container %s already exists', config.azure.containerName);
            }
                ...
                blobService.createBlockBlobFromStream(
                    ...
                    function(error, result, resp) {
                        if (error) {
							...
							file.url = config.azure.hostName + config.azure.containerName + '/' + file.name;
							response.statusCode = 200;
							response.writeHead(200, {
								'Content-Type': 'application/json'
							});
							response.end(file.url);
                        }//if
                    });//blobService.createBlockBlobFromStream
            } catch (ex) {
...
function serveStatic(response, pathname) {
    ...
    //else
        response.end(fs.readFileSync('./public/images' + pathname));
}
